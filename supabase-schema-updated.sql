-- NIL Matchup Database Schema (Updated)
...
-- Create athletes table (only if it doesn't exist)
...
-- Create deals table (only if it doesn't exist)
...
DROP POLICY IF EXISTS "Users can read their own athlete profile" ON athletes;
DROP POLICY IF EXISTS "Businesses can read athlete profiles" ON athletes;
DROP POLICY IF EXISTS "Users can update their own athlete profile" ON athletes;
DROP POLICY IF EXISTS "Users can delete their own athlete profile" ON athletes;

DROP POLICY IF EXISTS "Users can insert their own business profile" ON businesses;
DROP POLICY IF EXISTS "Users can read their own business profile" ON businesses;
DROP POLICY IF EXISTS "Athletes can read business profiles" ON businesses;
DROP POLICY IF EXISTS "Users can update their own business profile" ON businesses;
DROP POLICY IF EXISTS "Users can delete their own business profile" ON businesses;

DROP POLICY IF EXISTS "Athletes can create deals" ON deals;
DROP POLICY IF EXISTS "Users can read deals where they are the athlete" ON deals;
DROP POLICY IF EXISTS "Users can read deals where they are the business" ON deals;
DROP POLICY IF EXISTS "Businesses can update deals they're involved in" ON deals;
DROP POLICY IF EXISTS "Athletes can update their own deals" ON deals;
...
SELECT 
    table_name,
    CASE 
        WHEN table_name IN ('athletes', 'businesses', 'deals') THEN '✅ Created'
        ELSE '❌ Not Created'
    END as status
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('athletes', 'businesses', 'deals'); 